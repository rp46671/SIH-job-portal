<div class="container col-sm-8">
 <li class="list-group-item p-3">
  <div class="row">
              <div class="col">
                <div class="card card-small mb-4">
                  <div class="card-header border-bottom">
                    <h6 class="m-0">Skills Set Test</h6>
                  </div>
                  <div class="card-body p-0 pb-3 text-center">
                    <table class="table mb-0">
                      <tbody>
                        <tr>
                          <td><a href="<?php echo site_url('Test_Controller/network');?>"><span class="btn btn-accent" >Networking</span> </a></td>
                          <td><a href="<?php echo site_url('Test_Controller/network');?>"><span class="btn btn-accent" >Marketting<span></td>
                          <td><a href="<?php echo site_url('Test_Controller/network');?>"><span class="btn btn-accent" >IOS</span></td>
                          <td><a href="<?php echo site_url('Test_Controller/network');?>"><span class="btn btn-accent" >HR
                          </span></td>
                          <td><a href="<?php echo site_url('Test_Controller/network');?>"><span class="btn btn-accent" >Graphic Design</span></td>
                          <td><a href="<?php echo site_url('Test_Controller/network');?>"><span class="btn btn-accent" >Android</span></td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                </div>
              </div>
            </div>
            
                      <!--div class="row">
                        <div class="col-sm-12 col-md-6">
                          <strong class="text-muted d-block mb-2">Test</strong>
                          <br>
                            <div class="form-group mb-6">
                             <button type="submit" class="btn btn-accent" name="net" id="net" href="<?php echo site_url('Main/Networking_Test');?>">Networking</button>
                            
                                <button type="submit" name="market" id="market" class="btn btn-accent" href="<?php echo site_url('Main/Networking_Test');?>">Marketting</button>
                           
                                <button type="submit" name="ios" id="ios"class="btn btn-accent" href="<?php echo site_url('Main/Networking_Test');?>">iOS</button>
                            
                                <button type="submit" name="HR" id="HR"class="btn btn-accent"  href="<?php echo site_url('Main/Networking_Test');?>">HR</button>
                             
                              </div>
                                <button type="submit"  name="GraD" id="GraD" class="btn btn-accent"  href="<?php echo site_url('Main/Networking_Test');?>">Graphic Design</button>
                         
                                <button type="submit" name="And" id="And" class="btn btn-accent"  href="<?php echo site_url('Main/Networking_Test');?>">Android</button>
                        
                                <button type="submit" name="Data" id="Data" class="btn btn-accent"  href="<?php echo site_url('Main/Networking_Test');?>">Database</button>
                            
                                <button type="submit" name="front_end" id="front_end" class="btn btn-accent"  href="<?php echo site_url('Main/Networking_Test');?>">Front End</button>
                          
                                <button type="submit"  name="test" id="test"  class="btn btn-accent"  href="<?php echo site_url('Main/Networking_Test');?>">Testing</button>
                               </div-->
                        </div>
                      </div>
                    </li>
                  </div>
